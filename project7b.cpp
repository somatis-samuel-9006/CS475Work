...
main( int argc, char *argv[ ] )
{
	...
	if( me == BOSS )
	{
		for( int dst = 0; dst < NumCpus; dst++ )
		{
			if( dst == BOSS )
				continue;
            //num of elems to send to other cpus should = ppsize + maxshifts
            //I don't know about what tag i 'should' use, the slides use 'B' though
			MPI_Send( &BigSignal[dst*PPSize], PPSize+MAXSHIFTS, MPI_FLOAT, dst, 'B', MPI_COMM_WORLD );
		}
	}
	else
	{
        //others processors recieving their subsignals
	...
	{   
        //the number of elems to send back should be MAXSHIFTS i believe
        //here the dest should be boss
        //don't know about reusing tags, i'll try C this time
	...
	if( me == BOSS )
	{
		...
		for( int src = 0; src < NumCpus; src++ )
		{
			...
			MPI_Recv( tmpSums, MAXSHIFTS, MPI_FLOAT, src, 'C', MPI_COMM_WORLD, &status );
			for( int s = 0; s < MAXSHIFTS; s++ )
				BigSums[s] += tmpSums[s];
		}
	}
